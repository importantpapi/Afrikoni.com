...
-- Database Views and Functions for Intelligence Layers
-- ============================================================================
...
SELECT 
  ...
  -- Delivery Metrics (if shipments table has delivery data)
...
SELECT 
  ...
  COALESCE(SUM(CASE WHEN o.status = 'completed' THEN o.total_amount * 0.05 ELSE 0 END), 0) AS commission_earned, -- 5% commission
...
-- Demand Trends View (for seasonal patterns)
...
SELECT 
  ...
  -- Overall Risk Level (simplified calculation)
  CASE 
...
-- Trust Evolution View (tracks trust score changes over time)
...
SELECT 
  ...
  CASE 
    WHEN c.verified = true THEN 20 ELSE 0
...
-- Indexes for buyer_intelligence
CREATE INDEX IF NOT EXISTS idx_buyer_intelligence_company_id ON companies(id) WHERE role IN ('buyer', 'hybrid');
CREATE INDEX IF NOT EXISTS idx_buyer_intelligence_rfqs ON rfqs(buyer_company_id, created_at);
CREATE INDEX IF NOT EXISTS idx_buyer_intelligence_orders ON orders(buyer_company_id, status, created_at);

-- Indexes for supplier_intelligence
CREATE INDEX IF NOT EXISTS idx_supplier_intelligence_company_id ON companies(id) WHERE role IN ('seller', 'hybrid');
CREATE INDEX IF NOT EXISTS idx_supplier_intelligence_quotes ON quotes(supplier_company_id, created_at);
CREATE INDEX IF NOT EXISTS idx_supplier_intelligence_orders ON orders(seller_company_id, status, created_at);

-- Indexes for trade_performance
CREATE INDEX IF NOT EXISTS idx_trade_performance_date ON orders(created_at);
CREATE INDEX IF NOT EXISTS idx_trade_performance_status ON orders(status, created_at);

-- Indexes for demand_intelligence
CREATE INDEX IF NOT EXISTS idx_demand_intelligence_rfqs ON rfqs(category_id, status, created_at);
CREATE INDEX IF NOT EXISTS idx_demand_intelligence_products ON products(category_id, status);

-- Indexes for risk_signals
CREATE INDEX IF NOT EXISTS idx_risk_signals_disputes ON disputes(buyer_company_id, seller_company_id, status);
CREATE INDEX IF NOT EXISTS idx_risk_signals_conversations ON conversations(buyer_company_id, seller_company_id, last_message_at);
CREATE INDEX IF NOT EXISTS idx_risk_signals_orders ON orders(buyer_company_id, seller_company_id, status, updated_at);
...
-- Admin users should have access to all intelligence views
-- Regular users should only see their own company's data

-- Grant access to views (RLS will be enforced by underlying tables)
