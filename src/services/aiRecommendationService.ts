...
export const generateRecommendations = (systemState: any): Recommendation[] => {
    ...
    if (financial.escrowStatus === 'required' || financial.escrowStatus === 'pending') {
        recommendations.push({
            ...
            description: 'A pending trade is waiting for escrow funding. Fund now to avoid fulfillment delays.',
            icon: Wallet,
    ...
    if (tradeReadiness.status === 'ready') {
        recommendations.push({
            ...
            description: 'Market demand matches your supply profile. Review open RFQs.',
            icon: Zap,
