...
const PRIORITY_CONFIG = {
    urgent: {
        ...
        icon: AlertTriangle,
        color: 'red',
        bgClass: 'bg-os-red/10',
        borderClass: 'border-os-red/20',
    ...
    if (totalActions === 0) {
        return (
            <Surface variant="panel" className="p-12 text-center">
                ...
                <h3 className="text-os-xl font-bold text-os-text-primary mb-2">All Clear</h3>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between px-2">
                <div>
                    <h2 className="text-os-xl font-bold text-os-text-primary flex items-center gap-2">
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className={cn("text-os-xs font-bold uppercase tracking-[0.2em]", config.textClass)}>
...
function ActionItem({ action, onClick, config }) {
    ...
    return (
        <div
            className="group p-5 bg-os-surface-solid/50 hover:bg-os-surface-solid border border-os-stroke hover:border-os-accent/30 rounded-os-md transition-all duration-300 cursor-pointer shadow-subtle hover:shadow-os-md"
            onClick={onClick}
        >
            <div className="flex items-start justify-between gap-6">
                <div className="flex items-start gap-4 flex-1 min-w-0">
                    <div className={cn(
                        "w-10 h-10 rounded-os-sm flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-105",
                        config.bgClass
                    )}>
                    ...
                    <div className="flex-1 min-w-0 pt-0.5">
                        <h4 className="text-os-sm font-bold text-os-text-primary mb-1 group-hover:text-os-accent transition-colors">
                            {action.title}
                        </h4>
                        <p className="text-os-xs text-os-text-secondary leading-relaxed line-clamp-2">
                            {action?.description || 'No coordination details available.'}
                        </p>

                        {action?.metadata && (
                            <div className="flex items-center gap-4 mt-3 text-os-xs font-bold uppercase tracking-wider text-os-text-secondary/60">
                ...
                <div className="flex-shrink-0 pt-1">
                    ...
                    >
                        <span className="text-os-xs font-bold uppercase tracking-tighter">{action.actionLabel || 'Proceed'}</span>
