...
export default function TradeOSSidebar({
  ...
  const OSNavItem = ({ icon: Icon, label, path, badge, active, onClick }) => (
    ...
    >
      ...
      {/* Tooltip Label (Hover) */}
      <div className="absolute left-14 px-3 py-1.5 bg-[#1A1A1A] border border-[#333] rounded-lg text-[11px] font-medium text-white opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50 flex items-center gap-2 shadow-xl">
        {label}
        {badge > 0 && (
  ...
  return (
    <aside className={`
      fixed left-0 top-0 h-screen w-[72px] bg-[#050505] border-r border-[#1A1A1A]
      flex flex-col items-center py-5 z-[60] transition-transform duration-200
    ...
    `}>
      ...
      <div className="flex flex-col gap-4 w-full px-2 items-center mt-auto pb-4">
        ...
        <button onClick={onClose} className="md:hidden mt-2 p-2 text-gray-500 hover:text-white">
          <X className="w-5 h-5" />
        </button>
      </div>
    </aside>
