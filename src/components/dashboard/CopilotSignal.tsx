...
export const CopilotSignal = ({
    ...
    const tones = {
        ...
        express: { color: 'text-os-accent', bg: 'bg-os-accent/10', border: 'border-os-accent/20', icon: Zap }
    };
        ...
        <AnimatePresence>
            {active && (
                <motion.div
                    ...
                    className={cn(
                        "p-4 rounded-os-md border flex items-start gap-4 transition-all duration-500",
                        tone.bg, tone.border
                    )}
                >
                    <div className={cn("p-2 rounded-os-sm bg-black/40 border shadow-inner", tone.border)}>
                        <Icon className={cn("w-5 h-5", tone.color)} />
                    </div>
                    <div className="space-y-1">
                        <div className="flex items-center gap-3">
                            <span className={cn("text-os-xs font-black uppercase tracking-[0.2em]", tone.color)}>
                                {label}
                            </span>
                            {value && (
                                <span className="text-os-xs font-mono font-bold text-white/40 uppercase tracking-widest">
                                    {value}
                                </span>
                            )}
                        </div>
                        <p className="text-os-xs text-white/70 leading-relaxed italic font-medium">
                            {message}
                        </p>
                    </div>
                </motion.div>
