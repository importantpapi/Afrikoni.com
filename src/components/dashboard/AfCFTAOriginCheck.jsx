        ...
        <Surface variant="glass" className={`p-5 border ${isQualified ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-amber-500/20 bg-amber-500/5'}`}>
            <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className={`text-os-sm font-bold uppercase tracking-wide ${isQualified ? 'text-emerald-400' : 'text-amber-400'}`}>
                            {isQualified ? 'AfCFTA Compliance Verified' : 'Origin Check Pending'}
                        </h3>
                        <p className="text-os-xs text-white/60 mt-1 max-w-[280px]">
                            {compliance.message}
                        </p>
                    </div>
                </div>
            ...
            <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-2 gap-4">
                <div>
                    <div className="text-os-xs uppercase text-white/40 mb-1">Rule Applied</div>
                    <div className="text-os-xs text-white/90 font-medium">{compliance.rule_applied?.description}</div>
                </div>
                <div>
                    <div className="text-os-xs uppercase text-white/40 mb-1">Next Step</div>
                    {isQualified ? (
                        <div className="text-os-xs text-emerald-400 font-medium flex items-center gap-1">
                            Ready for Certificate <CheckCircle2 className="w-3 h-3" />
                        </div>
                    ) : (
                        <div className="text-os-xs text-amber-400 font-medium flex items-center gap-1 cursor-pointer hover:underline">
