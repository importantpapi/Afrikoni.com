    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-3 gap-3">
            {formData.imageUrls.map((url, index) => (
              ...
              >
                ...
                {index === 0 && (
                  <span className="absolute bottom-1.5 left-1.5 text-[10px] font-medium bg-white text-black px-1.5 py-0.5 rounded">
                    Main
                  </span>
                )}
              </div>
      ...
      <div className="space-y-2">
        <Label htmlFor="video" className="text-sm font-medium flex items-center gap-2">
          ...
          Product Video URL
          <span className="text-xs text-[var(--os-text-secondary)] font-normal">(optional)</span>
        </Label>
        <Input
          id="video"
          type="url"
        ...
        <div className="p-4 rounded-lg bg-white/5 border border-white/10">
          ...
          <ul className="space-y-1">
            {EXAMPLE_GOOD_LISTING.points.map((point, i) => (
              <li key={i} className="text-xs text-[var(--os-text-secondary)] flex items-start gap-1.5">
