...
export function PlatformSecurityIndicators({ variant = 'full', className = '' }) {
  ...
  if (variant === 'minimal') {
    return (
      <div className={`flex items-center gap-4 text-os-xs text-afrikoni-deep/60 ${className}`}>
    ...
    return (
      <div className={`grid grid-cols-3 gap-3 ${className}`}>
        {indicators.slice(0, 3).map((item, index) => {
          ...
          return (
            <div key={index} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white border border-os-accent/20">
              <Icon className={`w-4 h-4 text-${item.color}-600 flex-shrink-0`} />
              <div>
                <p className="text-os-xs font-medium text-afrikoni-chestnut">{item.label}</p>
              </div>
            </div>
          );
        })}
  ...
  return (
    <Card className={`border-os-accent/30 bg-gradient-to-br from-white to-amber-50 ${className}`}>
          ...
          {indicators.map((item, index) => {
            ...
            return (
              <div key={index} className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-os-sm font-medium text-afrikoni-chestnut">{item.label}</p>
                  <p className="text-os-xs text-afrikoni-deep/60 mt-0.5">{item.description}</p>
                </div>
              </div>
            );
          })}
        </div>
        <p className="text-os-xs text-afrikoni-deep/50 mt-4 pt-4 border-t border-os-accent/20 italic">
...
export function TrustBadgesFooter() {
  return (
    <div className="flex items-center justify-center gap-6 py-6 border-t border-os-accent/20">
      <div className="flex items-center gap-2 text-os-xs text-afrikoni-deep/60">
