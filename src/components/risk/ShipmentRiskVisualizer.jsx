    ...
    return (
        <Surface variant="panel" className="p-6 h-full flex flex-col">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-sm font-semibold uppercase tracking-wider text-[var(--os-text-primary)] flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-os-muted" />
                    Shipment Integrity
                </h3>
                <span className={cn("text-xs font-bold px-2 py-0.5 rounded border bg-white/5 border-white/5 uppercase", getRiskColor(riskAnalysis.level))}>
            ...
            <div className="flex items-center gap-6 mb-6">
                ...
                <div className="relative w-24 h-24 flex-shrink-0 flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-2xl font-bold text-white">{riskAnalysis.score}</span>
                        <span className="text-[10px] text-os-muted uppercase">Health</span>
                    </div>
                </div>
            ...
            <div className="space-y-2 flex-grow overflow-y-auto pr-1 custom-scrollbar">
                {riskAnalysis.alerts.map((alert, i) => {
                    ...
                    return (
                        <div key={i} className={cn("p-2 rounded border flex items-center gap-3 text-xs transition-colors", getSeverityBg(alert.severity))}>
                ...
                {riskAnalysis.alerts.length === 0 && (
                    <div className="text-center py-4 text-emerald-400/60 text-xs italic">
