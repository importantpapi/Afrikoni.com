          ...
          <Card className="border-afrikoni-gold/20 bg-white shadow-premium-lg rounded-2xl" style={{ position: 'relative', zIndex: 9000 }}>
            <CardContent className="p-4">
              ...
              {suggestions.length > 0 && (
                <div className="mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-semibold text-afrikoni-text-dark/70 uppercase">
                      Suggestions
                    </span>
                  </div>
                  <div className="space-y-1">
                    {suggestions.map((suggestion, idx) => (
                      <button
                        ...
                        className="w-full flex items-center justify-between p-2 rounded-lg hover:bg-afrikoni-gold/5 transition-colors text-left group"
                      >
                        <div className="flex items-center gap-2">
                          <Search className="w-4 h-4 text-afrikoni-deep/50 group-hover:text-afrikoni-gold" />
                          <span className="text-sm text-afrikoni-text-dark">{suggestion.text}</span>
                          <Badge variant="outline" className="text-xs">
                            {suggestion.type}
                          </Badge>
                        </div>
                        <ArrowRight className="w-4 h-4 text-afrikoni-deep/30 group-hover:text-afrikoni-gold opacity-0 group-hover:opacity-100 transition-opacity" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {showHistory && history.length > 0 && !query && (
                <div className="mb-4 border-t border-afrikoni-gold/10 pt-4">
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="space-y-1">
                    {history.map((item, idx) => (
                      <button
                        ...
                        className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-afrikoni-gold/5 transition-colors text-left"
                      >
              ...
              {showTrending && trending.length > 0 && !query && (
                <div className="border-t border-afrikoni-gold/10 pt-4">
                  <div className="flex items-center gap-2 mb-2">
              ...
              {isLoading && (
                <div className="text-center py-4">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-afrikoni-gold mx-auto" />
                </div>
              )}
            </CardContent>
          </Card>
