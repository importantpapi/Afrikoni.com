    ...
    <Surface variant="panel" className="p-5 h-full">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Delay risk & compliance rate by lane</p>
        </div>
        ...
        {corridors.map((c) => (
          <div key={c.id} className="p-3 rounded-lg border border-border/60 bg-card/80">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-xs text-muted-foreground">{c.trades} trades • ${Math.round(c.totalValue/1000)}k</span>
            </div>
