    ...
    return (
        <div className="w-full h-full bg-white/80 dark:bg-[#08090A]/90 backdrop-blur-3xl border-l border-white/20 dark:border-white/5 shadow-2xl z-[100] flex flex-col font-sans">
                ...
                <div className="flex items-center justify-between mb-2 relative z-10">
                    <div className="flex items-center gap-3">
                        <div className="relative group">
                            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center text-white shadow-[0_0_25px_rgba(99,102,241,0.4)] border border-white/20 transition-all duration-500 group-hover:scale-110 group-hover:rotate-3">
                                <Sparkles className="w-5 h-5 animate-pulse" />
                            </div>
                            <div className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-emerald-400 border-[3px] border-white dark:border-black rounded-full shadow-lg z-20" />
                        </div>
                        <div>
                            <span className="font-black text-lg text-transparent bg-clip-text bg-gradient-to-r from-gray-900 via-indigo-800 to-gray-900 dark:from-white dark:via-indigo-200 dark:to-white tracking-tight block leading-none mb-1">
                                KoniAI
                            </span>
                            <div className="flex items-center gap-2">
                                <div className="h-1.5 w-1.5 rounded-full bg-indigo-500 animate-[ping_2s_ease-in-out_infinite]" />
                                <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">v4.0 Live</span>
                            </div>
                        </div>
                    </div>
                    <button
            ...
            <div className="flex-1 overflow-y-auto p-5 custom-scrollbar scroll-smooth" ref={scrollRef}>
                ...
                {messages.length === 0 && (
                    <div className="mb-6 animate-in slide-in-from-bottom-4 duration-700 fade-in fill-mode-forwards">
                        <div className="p-1 rounded-2xl bg-gradient-to-r from-indigo-500/20 via-purple-500/20 to-pink-500/20 mb-6">
                            <div className="bg-white/50 dark:bg-black/40 backdrop-blur-md rounded-xl p-4 border border-white/10 text-center">
                        ...
                        {recommendations.length > 0 ? (
                            ...
                            <div className="text-center py-10 text-gray-400 text-xs font-medium border border-dashed border-gray-200 dark:border-white/10 rounded-2xl bg-gray-50/50 dark:bg-white/5">
                                All systems nominal.<br />
                                <span className="text-[10px] opacity-60">Monitoring trade networks...</span>
                            </div>
                        )}

                        <div className="mt-8">
                            <p className="text-[10px] font-bold text-center text-gray-400 uppercase tracking-widest mb-3">Quick Actions</p>
                            <div className="flex flex-wrap justify-center gap-2">
                                {["Draft a cocoa RFQ", "Check shipping rates", "Analyze my risk"].map(q => (
                                    <button
                                        key={q}
                                        onClick={() => { setInputValue(q); }}
                                        className="text-[11px] font-medium px-4 py-2 rounded-xl bg-white dark:bg-white/5 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 border border-gray-100 dark:border-white/10 hover:border-indigo-200 dark:hover:border-indigo-500/50 transition-all text-gray-600 dark:text-gray-300 shadow-sm hover:shadow-md hover:-translate-y-0.5"
                                    >
                                        {q}
                                    </button>
                                ))}
                ...
                <div className="space-y-6 pb-4">
                    {messages.map((msg) => (
                        ...
                        >
                            <div className={cn(
                                "p-4 sm:p-5 rounded-2xl text-sm leading-7 shadow-sm relative overflow-hidden group transition-all duration-300",
                                msg.role === 'user'
                                ...
                                <div className="mt-3 flex flex-wrap gap-2 animate-in fade-in slide-in-from-top-2 duration-500">
                                    {msg.actions.map((action, idx) => (
                                        <Button
                                            ...
                                            className="text-xs h-8 bg-white/50 dark:bg-white/5 border-gray-200 dark:border-white/10 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:border-indigo-200 dark:hover:border-indigo-500/30 transition-all rounded-lg"
                                        >
                                            {action.label}
                                        </Button>
                                    ))}
                    ...
                    {isTyping && (
                        <div className="flex items-center gap-1.5 ml-2 p-3 bg-white/50 dark:bg-white/5 rounded-2xl w-fit border border-gray-100 dark:border-white/5 backdrop-blur-sm animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-5 bg-gradient-to-t from-white via-white/95 to-transparent dark:from-[#08090A] dark:via-[#08090A]/95 flex-shrink-0 relative z-20 backdrop-blur-sm">
                <div className="relative shadow-[0_0_40px_rgba(0,0,0,0.08)] rounded-3xl bg-white dark:bg-[#15171A] border-2 border-transparent focus-within:border-indigo-500/20 transition-all duration-300">
                    {/* Input Glow */}
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-3xl opacity-0 focus-within:opacity-20 transition-opacity duration-300 blur-md pointer-events-none" />
                    ...
                    <input
                        ...
                        disabled={isTyping}
                        className="w-full relative bg-transparent border-none py-4 pl-6 pr-14 text-sm font-medium text-gray-900 dark:text-white focus:ring-0 placeholder:text-gray-300 dark:placeholder:text-gray-600 disabled:opacity-50 transition-all"
                    />
                    <div className="absolute right-2 top-1/2 -translate-y-1/2">
                        <button
                            onClick={handleSend}
                            disabled={!inputValue.trim() || isTyping}
                            className="p-2.5 bg-black dark:bg-white text-white dark:text-black rounded-2xl hover:scale-105 active:scale-95 transition-all disabled:opacity-30 disabled:hover:scale-100 shadow-lg hover:shadow-xl"
                        >
                ...
                <div className="mt-3 flex justify-center items-center gap-1.5 opacity-40">
                    ...
                    <span className="text-[9px] font-bold text-gray-500 uppercase tracking-[0.25em]">Neural Engine v4.0</span>
                    <div className="w-1 h-1 rounded-full bg-gray-500" />
                </div>
            </div>
        </div>
