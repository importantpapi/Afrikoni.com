        ...
        {steps.map((step, idx) => {
          ...
          return (
            ...
            >
              {/* Connector line */}
              {idx < steps.length - 1 && (
                <div className="hidden lg:block absolute top-12 left-full w-full h-0.5 bg-afrikoni-gold/20 z-0" style={{ width: 'calc(100% - 3rem)' }} />
              )}
              <div className="bg-afrikoni-cream rounded-xl border border-afrikoni-gold/30 p-6 h-full relative z-10">
          ...
          return (
            ...
            >
              <div className="flex-shrink-0 flex flex-col items-center">
                <div className="w-12 h-12 rounded-full bg-afrikoni-gold text-white flex items-center justify-center font-bold text-lg">
                  {step.number || idx + 1}
                </div>
                {idx < steps.length - 1 && (
                  <div className="w-0.5 h-full bg-afrikoni-gold/20 mt-2" style={{ minHeight: '2rem' }} />
                )}
              </div>
              <div className="flex-1 bg-afrikoni-cream rounded-xl border border-afrikoni-gold/30 p-6">
                {Icon && (
                  <div className="w-10 h-10 rounded-lg bg-afrikoni-gold/20 flex items-center justify-center mb-4 inline-flex">
                    <Icon className="w-5 h-5 text-afrikoni-gold" />
                  </div>
                )}
                <h3 className="text-h3 font-semibold leading-[1.3] text-afrikoni-chestnut mb-2">
