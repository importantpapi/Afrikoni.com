    ...
    return (
        <div className="flex flex-col gap-4 p-4 pb-24 bg-black text-[#E8E8E8] font-sans antialiased">
            ...
            <div className="border-b border-[#222] pb-4 mb-2">
                <p className="text-[#888] text-os-xs uppercase font-bold tracking-widest leading-none mb-1">Trade ID: {trade?.id?.slice(0, 8)}</p>
                <h1 className="text-os-2xl font-black text-white">{trade?.title || 'ONE FLOW ENTRY'}</h1>
            </div>

            {/* Status Indicator */}
            <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#111] border border-[#333] p-4 rounded-os-sm">
                    <p className="text-os-xs text-[#888] uppercase font-black mb-1">Status</p>
                    <p className="text-os-accent font-bold text-os-sm tracking-tight">{status.toUpperCase()}</p>
                </div>
                <div className="bg-[#111] border border-[#333] p-4 rounded-os-sm">
                    <p className="text-os-xs text-[#888] uppercase font-black mb-1">Verify DNA</p>
                    <p className="text-emerald-500 font-bold text-os-sm">SECURE</p>
                </div>
            </div>
...
function MobileActionItem({ icon, title, desc, color }) {
    return (
        <button className="w-full flex items-center justify-between p-5 bg-[#111] hover:bg-[#161616] border border-[#222] rounded-os-md active:scale-[0.98] transition-all">
            <div className="flex items-center gap-4">
                <div className={`${color}`}>
                    {icon}
                </div>
                <div className="text-left">
                    <p className="font-black text-os-sm text-white tracking-tight">{title}</p>
                    <p className="text-os-xs text-[#666] font-bold uppercase tracking-wider">{desc}</p>
                </div>
            </div>
